...
export const RecomBlockVacancy: React.FC<RecomApp> = ({ data }) => {
    ...
    return (
        <div className="bg-white shadow-md last:mb-0 rounded-xl w-full h-full" key={data.id}>
            <Link to={`/vacancies/${data.slug}`} className='h-full inline-block'>
                <div className="p-6 flex flex-col items-start">
                    <h1 className="lg:text-2xl font-bold max-sm:text-lg sm:text-lg md:text-xl">{data[getValueByLanguage("title")]}</h1>
                    <div className="lg:text-base text-[#777B80] my-6 font-normal max-sm:text-xs sm:text-xs md:text-sm text-foreground line-clamp-4" dangerouslySetInnerHTML={{__html: data[getValueByLanguage("content")]}}></div>
